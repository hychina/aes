...
if __name__ == '__main__':
	if len(sys.argv) != 2:
		print 'I need the file path!'
		exit(1)
	file_object = open(sys.argv[1])
	rater_human = [[] for i in range(8)]
	rater_computer = [[] for i in range(8)]
	try:
		one_line = file_object.readline()
		while one_line:
			parts = one_line.split()
			if len(parts) != 3:
				print 'length of column should be 3! (test_set, human_score, computer_score)'
				exit(1)
			idx = string.atoi(parts[0]) - 1
			rater_human[idx].append(string.atoi(parts[1]))
			rater_computer[idx].append(string.atoi(parts[2]))
			one_line = file_object.readline()
	finally:
		file_object.close()
	
	#print rater_human
	#print rater_computer
	
	kappas = []
	for i in range(8):
		if len(rater_human[i]) == 0:
			continue
		kappas.append(quadratic_weighted_kappa(rater_human[i], rater_computer[i]))
	
	#print kappas
	
	final_score = mean_quadratic_weighted_kappa(kappas)
